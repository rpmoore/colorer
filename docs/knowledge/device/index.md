...
- [HID set-color path](set-color-hid.md) — `set_color_impl`'s revalidation/retry machinery; two hardware-verified `IMPLEMENTED_PROTOCOLS` entries: Razer Ornata V3 (Feature report, empirically-required report-ID prefix byte) and Gigabyte RGB Fusion 2's CPU ARGB strip (Gen2 addressable-strip protocol, one header only)
